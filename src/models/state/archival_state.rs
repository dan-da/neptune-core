...
mod archival_state_tests {
    ...
    async fn initialize_archival_state_test() -> Result<()> {
        ...
        let (block_1, _, _) =
            make_mock_block_with_valid_pow(&b, None, some_receiving_address.into(), rng.gen());
    ...
    async fn update_mutator_set_rollback_ms_block_sync_multiple_inputs_outputs_in_block_test() {
        ...
        let addition_records_1a = ars(magic_number);
        let removal_records_1b = rrs(0);
        let addition_records_1b = ars(magic_number);

        let tx_1a = make_mock_transaction_with_mutator_set_hash(
            removal_records_1a,
            addition_records_1a,
            genesis_block.mutator_set_accumulator_after().hash(),
        );
        ...
        assert_eq!(
            num_premine_utxos + magic_number + block_1b.guesser_fee_utxos().len(),
            global_state_lock
                .lock_guard()
                .await
                .chain
    ...
    async fn allow_multiple_inputs_and_outputs_in_block() {
        ...
        assert_eq!(
            ...
            // = 20 + 128 - 10 - 5 - 1
            // = 132
            Block::block_subsidy(block_1.header().height) + NeptuneCoins::new(20 - 10 - 5 - 1),
            genesis
                .lock_guard()
                .await
                .get_wallet_status_for_tip()
    ...
    async fn ms_update_to_tip_five_blocks() {
        ...
        for _block_height in 1..=5 {
            let next_block =
                make_mock_block(&current_block, None, cb_beneficiary.into(), rng.gen()).0;
    ...
    async fn ms_update_to_tip_fork_depth_1() {
        ...
        let block_1a = make_mock_block(&genesis_block, None, cb_beneficiary.into(), rng.gen()).0;
        let block_1b = make_mock_block(&genesis_block, None, cb_beneficiary.into(), rng.gen()).0;
    ...
    async fn ms_update_to_tip_fork_depth_2() {
        ...
        let block_1a = make_mock_block(&genesis_block, None, cb_beneficiary.into(), rng.gen()).0;
        let block_2a = make_mock_block(&block_1a, None, cb_beneficiary.into(), rng.gen()).0;
        let block_1b = make_mock_block(&genesis_block, None, cb_beneficiary.into(), rng.gen()).0;
        let block_2b = make_mock_block(&block_1b, None, cb_beneficiary.into(), rng.gen()).0;
        let block_1a_msa = &block_1a.mutator_set_accumulator_after();
    ...
    async fn fork_path_finding_test() -> Result<()> {
        ...
        let own_receiving_address = own_wallet
            .nth_generation_spending_key_for_tests(0)
            .to_address();
        let (block1, _, _) = make_mock_block(
            &genesis.clone(),
            None,
            own_receiving_address.into(),
            rng.gen(),
