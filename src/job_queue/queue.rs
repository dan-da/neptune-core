...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread")]
    #[traced_test]
    async fn find_n_primes() -> anyhow::Result<()> {
        workers::find_n_primes().await
    }
    ...
    mod workers {
        ...
        pub(super) async fn find_n_primes() -> anyhow::Result<()> {
            use rand::Rng;
            ...
            // other async tasks.
            impl FindPrimesJob {
                async fn is_prime(num: u128) -> bool {
                    if num <= 1 {
                        return false;
                    }
                    if num <= 3 {
            ...
            impl Job for FindPrimesJob {
                ...
                async fn run_async(&self) -> Box<dyn JobResult> {
                    ...
                    FindPrimesJobResult::from(found_primes).into()
                }
            }

            // setup
            const NUM_PRIMES_PER_JOB: u128 = 100;
