...
pub struct JobQueue<P: Ord> {
    tx: mpsc::UnboundedSender<((Box<dyn Job>, JobResultOneShotChannel), P)>,
}
...
impl<P: Ord + Send + Sync + 'static> JobQueue<P> {
    ...
    pub fn start() -> Self {
        let (tx, mut rx) =
            mpsc::unbounded_channel::<((Box<dyn Job>, JobResultOneShotChannel), P)>();

        let jobs: Arc<Mutex<VecDeque<((Box<dyn Job>, JobResultOneShotChannel), P)>>> =
    ...
    mod workers {
        ...
        pub(super) fn runtime_shutdown_cancels_job(is_async: bool) -> anyhow::Result<()> {
            ...
            let result = rt.block_on(async {
                ...
                let _ = job_queue.add_job(job, DoubleJobPriority::Low).await?;
